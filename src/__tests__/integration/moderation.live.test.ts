...
suite("live: moderation, users and app", () => {
  ...
  beforeAll(async () => {
    ...
    harness.onCleanup(() =>
      harness.call("users_delete", {
        user_ids: [moderator, offender],
        user: "hard",
        messages: "hard",
      })
    );
    ...
    harness.onCleanup(() => harness.call("chat_delete_channel", { ...channel, hard_delete: true }));
  ...
  it("flags a message and finds it in the review queue", async () => {
  ...
  it("deactivates and reactivates a user", async () => {
    ...
    harness.onCleanup(() => harness.call("users_delete", { user_ids: [temp], user: "hard" }));
  ...
  it("creates a guest user", async () => {
    ...
    harness.onCleanup(() =>
      harness.call("users_delete", { user_ids: [result.user.id], user: "hard" })
    );
  ...
  it("reads app settings, rate limits and channel/call types", async () => {
    ...
    expect(Object.keys(channelTypes.channel_types).length).toBeGreaterThan(0);
    ...
    expect(Object.keys(callTypes.call_types)).toContain("default");
