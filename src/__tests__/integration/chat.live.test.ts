...
suite("live: chat", () => {
  ...
  beforeAll(async () => {
    ...
    harness.onCleanup(() =>
      harness.call("users_delete", { user_ids: [alice, bob], user: "hard", messages: "hard" })
    );
  }, 60_000);
  ...
  it("pins a message and lists pinned messages", async () => {
    ...
    expect(pinned.messages.map((m: any) => m.id)).toContain(messageId);
  });
  ...
  it("adds and removes members", async () => {
    ...
    harness.onCleanup(() =>
      harness.call("users_delete", { user_ids: [carol], user: "hard", messages: "hard" })
    );
  ...
  it("sends a custom channel event", async () => {
    ...
    expect(result.event.type).toBe("mcp.test");
  });
