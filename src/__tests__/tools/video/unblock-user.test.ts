...
import { registerUnblockUser } from "../../../tools/video/unblock-user.js";
...
describe("video_unblock_user", () => {
  ...
  it("unblocks a user from a call", async () => {
    const mockResponse = { duration: "200ms" };
    mockGetClient.mockReturnValue({
      video: { unblockUser: mockUnblockUser.mockResolvedValue(mockResponse) },
    });

    const tool = server["_registeredTools"]["video_unblock_user"];
