...
const deleteChannel = defineTool({
  ...
  handler: async (args, client) =>
    client.chat.deleteChannel({
      type: args.channel_type,
      id: args.channel_id,
      ...defined({ hard_delete: args.hard_delete }),
    }),
});
  ...
  handler: async (args, client) =>
    client.chat.truncateChannel({
      ...
      ...defined({
        ...
        truncated_at: args.truncated_at ? new Date(args.truncated_at) : undefined,
        message:
...
const sendEvent = defineTool({
  ...
  inputSchema: {
    ...channelRef,
    event_type: z.string().min(1).describe("Event type, e.g. 'typing.start' or a custom name"),
