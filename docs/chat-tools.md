...
| `channel_type` | string | **yes** | Channel type (e.g. 'messaging', 'team', 'livestream') |
| `channel_id` | string | **yes** | Channel ID |
| `event_type` | string | **yes** | Event type, e.g. 'typing.start' or a custom name |
| `user_id` | string | no | User the event is attributed to |
| `custom` | object | no | Custom key/value data stored on the object |
...
List the pinned messages in a channel, newest pin first.
...
| `banned_by_id` | string | no | Moderator performing the ban |
