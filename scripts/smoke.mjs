...
const { tools: appOnly } = await listTools({ STREAM_MCP_TOOLSETS: "app" });
assert(
  appOnly.length > 0 && appOnly.every((tool) => tool.name.startsWith("app_")),
  "STREAM_MCP_TOOLSETS=app registers only app tools"
);
