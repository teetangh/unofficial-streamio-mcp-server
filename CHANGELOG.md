...
- **89 new tools**, including capabilities that were previously impossible: reading messages (`chat_get_channel`, `chat_get_message`, `chat_get_replies`), searching chat (`chat_search_messages`), minting call-scoped tokens (`auth_create_call_token`), and taking a livestream live (`video_go_live`).
- Reactions, threads, read state, channel and call types, participants, closed captions, HLS and RTMP broadcasting, the moderation review queue, blocklists, app settings and rate limits.
